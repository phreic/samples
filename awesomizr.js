...
Awesomizr.createTableOfContents = function(params) {
    ...
    if (typeof params.toctitle !== 'undefined') {
        tocTitle = params.toctitle;
    } else {
        tocTitle = tocDefaultParams.toctitle;
    }
    ...
    var accessible = params.accessible || tocDefaultParams.accessible;
    ...
    for (var i = 0; i < elements.length; i++) {
        ...
        toc += '<a href="#' + id + '">' + text.replace("&", "&amp;").replace("<", "&lt;") + '</a></div>';
        if (accessible) {
            toc += '</div>';
        }
    }
